...
export default function App() {
  ...
  // FAB Kana hold / toggle handlers
  const activateKana = () => {
    setIsFabPressed(true);
    setShowKana(true);
  };

  const deactivateKana = () => {
    setIsFabPressed(false);
    setShowKana(false);
  };

  const toggleKanaClick = () => {
  ...
  }, [showJsonModal, activeTab, handleFlip, handleNext, handlePrev, handleToggleMode, currentCard]);
    ...
    <div className="app">
      ...
      {activeTab === 'flashcard' && (
        <>
          ...
          >
            ...
            >
              ...
              <section className="face back" id="back">
                ...
                <div className="ex-jp">{currentCard.example}</div>
          ...
          {/* Floating Action Button for Kana (Click to toggle or hold to reveal) */}
          <button
            className={`fab ${isFabPressed || showKana ? 'pressed' : ''}`}
            id="kanaHoldBtn"
            title="Bấm hoặc giữ để xem Kana"
            onClick={toggleKanaClick}
            onMouseDown={activateKana}
            onMouseUp={deactivateKana}
            onMouseLeave={deactivateKana}
            onTouchStart={(e) => {
              e.preventDefault();
              activateKana();
            }}
            onTouchEnd={deactivateKana}
            onTouchCancel={deactivateKana}
          >
            あ
          </button>
        </>
                ...
                >
                  ...
                  <div style={{ fontSize: '13px', color: '#6b7280', background: '#f9fafb', padding: '8px 10px', borderRadius: '10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ color: '#111827' }}>{w.example}</span>
                      <button
                        type="button"
                        className="audio-btn"
      ...
      {/* Quiz View */}
      {activeTab === 'quiz' && (
        <main className="simple-view-wrap" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          {!quizFinished ? (
                ...
                <div style={{ textAlign: 'center', padding: '16px 0 24px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                    <span style={{ fontSize: 'clamp(28px, 7vw, 42px)', fontWeight: '700', color: '#111827' }}>
                      {mode === 'jp-vi' ? quizCard.kanji : quizCard.viet}
                    </span>
                    {mode === 'jp-vi' && (
                      <button
                        type="button"
                  ...
                  {mode === 'jp-vi' && (
                    <div style={{ fontSize: '14px', color: '#6b7280', marginTop: '4px' }}>
                      【{quizCard.kana}】 • <span style={{ color: '#1d4ed8', fontWeight: '600' }}>{quizCard.hanViet}</span>
                    </div>
                  )}
                </div>
